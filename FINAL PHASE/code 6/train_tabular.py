...
train_tabular_q.py
==================
Tabular Q-learning with hand-crafted state abstraction for OBELIX.

Inspired by Mahadevan & Connell (1991) — the original paper this 
assignment is based on. They used Q-learning + state clustering on 
a real robot with similar binary sensors.

Key idea: map the 18-bit observation to ~20 meaningful abstract states
instead of feeding raw bits to a neural net. The state space is tiny
so Q-learning converges fast and reliably.

State abstraction:
  - Phase: FIND (no sensors active) / APPROACH (sensors active) / 
           PUSH (attached to box) / UNWEDGE (stuck at boundary)
  - Direction: where is the box relative to robot (left/front/right/behind)
  - Distance: near vs far
  - Stuck: wall contact

This gives a manageable ~30-40 state space that Q-learning can handle
in a few hundred episodes.

Usage
-----
    python train_tabular_q.py --episodes 5000 --mode curriculum
    python train_tabular_q.py --episodes 3000 --mode mixed
"""
...
# Sensor indices in the 18-bit observation vector
# Based on environment description:
# bits 0-7:  sonar near (8 sensors around robot)
# bits 8-15: sonar far  (8 sensors around robot)  
# bit 16:    infrared (directly ahead)
# bit 17:    stuck_flag (wall contact)

# Sonar layout (approximate, adjust if your env differs):
# Sensors 0-3: front quadrant (near), 4-5: sides (near), 6-7: rear (near)
# Sensors 8-11: front quadrant (far), 12-13: sides (far), 14-15: rear (far)

FRONT_NEAR  = [0, 1, 2, 3]   # front near sensors
SIDE_NEAR   = [4, 5, 6, 7]   # side/rear near sensors  
FRONT_FAR   = [8, 9, 10, 11] # front far sensors
SIDE_FAR    = [12, 13, 14, 15]# side/rear far sensors
IR_BIT      = 16              # infrared sensor
STUCK_BIT   = 17              # wall contact flag
...
def get_abstract_state(obs):
    """
    Map 18-bit raw observation to a small discrete state tuple.
    
    Returns a tuple that uniquely identifies the abstract state.
    Total states: ~32-40 (easily manageable for tabular Q)
    
    State dimensions:
      phase    : 0=blind/find, 1=far_detect, 2=near_detect, 3=ir_detect
      box_side : 0=front, 1=left, 2=right, 3=behind/unknown
      stuck    : 0=free, 1=wall_contact
    """
    ...
    left_active  = int(obs[4] or obs[5] or obs[12] or obs[13])   # left side sensors
    right_active = int(obs[6] or obs[7] or obs[14] or obs[15])   # right side sensors

    # ── phase ─────────────────────────────────────────────────────────────────
    if ir:
        phase = 3          # IR active = very close, about to attach
    elif front_near or any(obs[i] for i in SIDE_NEAR):
        phase = 2          # near detection
    elif front_far or any(obs[i] for i in SIDE_FAR):
        phase = 1          # far detection only
    else:
        phase = 0          # completely blind, exploring

    # ── box direction relative to robot ───────────────────────────────────────
    if ir or front_near or front_far:
        box_side = 0       # box is ahead
    elif left_active and not right_active:
        box_side = 1       # box is to the left
    elif right_active and not left_active:
        box_side = 2       # box is to the right
    else:
        box_side = 3       # unknown / behind

    return (phase, box_side, stuck)


def state_to_idx(state):
    """Convert abstract state tuple to integer index."""
    phase, box_side, stuck = state
    return phase * 8 + box_side * 2 + stuck

N_STATES = 4 * 4 * 2  # phase(4) × box_side(4) × stuck(2) = 32
...
def shape_reward(raw_reward, obs, prev_obs):
    ...
    # Reward approaching box (phase improvement)
    prev_phase = get_abstract_state(prev_obs)[0]
    curr_phase = get_abstract_state(obs)[0]
    if curr_phase > prev_phase:
        shaped += 50 * (curr_phase - prev_phase)   # big bonus for getting closer
    elif curr_phase < prev_phase:
        shaped -= 20  # penalty for losing track of box

    # Small penalty for being blind (encourages exploration)
    if curr_phase == 0:
        shaped -= 0.5
...
def train(args):
    ...
    for ep in range(1, args.episodes + 1):
        ...
        # learning rate decays over training
