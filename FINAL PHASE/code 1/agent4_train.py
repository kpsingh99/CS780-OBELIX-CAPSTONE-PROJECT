...
for OBELIX Warehouse Robot — CS780 Final Phase

Trains across ALL 3 difficulty levels (curriculum / mixed sampling)
so the saved weights.pth generalises to the test phase.

Usage
-----
    python train_d3qn_per.py                     # default mixed training
    python train_d3qn_per.py --level 3           # train only on level 3
    python train_d3qn_per.py --wall              # enable wall obstacles
    python train_d3qn_per.py --curriculum        # start easy, ramp up

After training, copy  weights.pth + agent.py  into submission.zip
"""
...
# ── Import the OBELIX environment ──────────────────────────────────────────────
# Make sure you run this script from inside the CS780-OBELIX repo root
from obelix import OBELIX  # adjust import if your env class is named differently

# ══════════════════════════════════════════════════════════════════════════════
# 1.  NETWORK  (identical to agent.py so saved weights load directly)
# ══════════════════════════════════════════════════════════════════════════════

OBS_DIM       = 18
OBS_STACK     = 4          # stack last 4 observations  →  72-dim input
...
# ══════════════════════════════════════════════════════════════════════════════
# 2.  PRIORITIZED EXPERIENCE REPLAY  (SumTree-based)
# ══════════════════════════════════════════════════════════════════════════════
...
class ObsStack:
    ...
    def _get(self):
        return np.concatenate(list(self.stack), axis=0)   # shape (72,)
...
EPS_END      = 0.05
EPS_DECAY_EP = 0.992
EPS_DECAY    = 0.999      # multiplicative decay per step
TARGET_UPDATE= 500         # hard update every N steps
MAX_EPISODES = 800        # total training episodes
MAX_STEPS    = 500       # matches evaluation horizon
SAVE_EVERY   = 200         # save checkpoint every N episodes
WARMUP_STEPS = 1000        # fill buffer before training starts


# ══════════════════════════════════════════════════════════════════════════════
# 6.  CURRICULUM / LEVEL SAMPLING
# ══════════════════════════════════════════════════════════════════════════════
...
# ══════════════════════════════════════════════════════════════════════════════
# 7.  REWARD SHAPING  (applied during training only, NOT at eval time)
# ══════════════════════════════════════════════════════════════════════════════
...
def shape_reward(raw_reward, obs, prev_obs, done, success):
    ...
    # Small bonus for keeping any sensor active (encourages approaching box)
    if any(obs[:16]):
        shaped += 0.1

    # Penalty for being completely blind two steps in a row
...
def train(args):
    ...
    for ep in range(1, MAX_EPISODES + 1):
        ...
        # ── reset environment ─────────────────────────────────────────────────
        # Adjust keyword args to match your ObelixEnv API
        env_kwargs = dict(
            level=level,
        ...
        for step in range(MAX_STEPS):
            ...
            ep_reward += reward   # log RAW reward for fair comparison
...
# ══════════════════════════════════════════════════════════════════════════════
# 9.  ENV WRAPPERS  (thin wrappers — adjust to your actual env API)
# ══════════════════════════════════════════════════════════════════════════════

_env = None   # global env handle reused across episodes

def env_reset(args, level):
    """Reset (or create) the OBELIX env and return initial observation."""
    global _env
    
    # OBELIX sets difficulty in __init__, so we recreate the environment 
    # to support the mixed/curriculum level changes.
    _env = OBELIX(
        scaling_factor=5,
        difficulty=level,
...
def env_step(env, action_str):
    ...
    """
    # obelix.py returns exactly 3 values
    obs, rew, done = env.step(action_str, render=False)
    
    # Create a dummy info dict since obelix doesn't return one.
    # We infer "success" if the agent receives the massive +2000 success bonus.
...
if __name__ == "__main__":
    ...
    args.env = None   # will be set in env_reset()
