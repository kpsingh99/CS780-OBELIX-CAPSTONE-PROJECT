"""Offline trainer: Double DQN + replay buffer (CPU) for OBELIX.
   TIER 1 IMPROVEMENTS APPLIED:
     1. Reward shaping       — dense signal during Find phase (sensor delta bonus)
     2. Observation stacking — last N obs concatenated to handle POMDP memory
     3. Dueling network      — separate V(s) and A(s,a) streams

Run locally to create weights.pth, then submit agent.py + weights.pth.

Example:
  python train_dqn_tier1.py --obelix_py ./obelix.py --out weights.pth --episodes 4000 --difficulty 0

                        ALGORITHM: DOUBLE DEEP Q-NETWORK (DDQN)
                        + TIER 1 IMPROVEMENTS (see sections marked [TIER1])

Double DQN splits:
• Online  network  → SELECT best action in next state
• Target  network  → EVALUATE that action's value

target = r + γ × Q_target( s', argmax_a Q_online(s', a) )

[TIER1-3] Dueling heads further decompose:
  Q(s,a) = V(s) + A(s,a) - mean_a(A(s,·))
  V-stream learns state value; A-stream learns action advantages.
  Helps in the Find phase where most rotations have equal state value.
"""
...
class ObsStack:
    """Keeps the last `n` observations and exposes them as a flat vector."""
    ...
    def reset(self, first_obs: np.ndarray | None = None):
        """Fill the buffer with zeros (or replicate first_obs if provided)."""
        self.buf.clear()
    ...
    def push(self, obs: np.ndarray) -> np.ndarray:
        """Add obs and return the current stacked vector."""
        self.buf.append(obs.astype(np.float32))
        return self.get()

    def get(self) -> np.ndarray:
        """Return concatenation of all buffered observations, shape (n*obs_dim,)."""
...
def shape_reward(
    ...
    """Return env_reward + a small bonus for newly activated sensor bits."""
    bonus = 0.0
